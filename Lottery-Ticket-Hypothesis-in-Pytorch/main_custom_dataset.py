...
# dataset = ShapesDataset("shapes/", transforms.ToTensor())
...
# tdata = ShapesDataset('/localhome/asa420/Lottery-Ticket-Hypothesis-in-Pytorch/data/shapes/', None)
#
# tloader = DataLoader(tdata, batch_size=16, shuffle=True)
...
def main(args, ITE=0):
    ...
    #     traindataset = datasets.shapes('./data', train=True, download=True,transform=transform)
    #     testdataset = datasets.CIFAR100('./data', train=False, transform=transform)
    #     from archs.shapes import resnet


    elif args.dataset == 'shapes':
        dataset = ShapesDataset("/localhome/asa420/Lottery-Ticket-Hypothesis-in-Pytorch/data/shapes/", transforms.ToTensor())
        # dataset = ShapesDataset('/localhome/asa420/Lottery-Ticket-Hypothesis-in-Pytorch/data/shapes/', None)

        train_dataset_num = int(len(dataset)*0.7)
        test_dataset_num = int(len(dataset)*0.3)
        # test_dataset_num = len(dataset) - val_dataset_num - train_dataset_num
        ...
        # traindataset, testdataset = random_split(dataset, [0.7, 0.3], generator=torch.Generator().manual_seed(42))
        from archs.shapes import resnet
    ...
    # train_loader = DataLoader(traindataset, 16, shuffle=True)
    # # val_loader = DataLoader(valdataset, 16, shuffle=False)
    # test_loader = DataLoader(testdataset, 16, shuffle=False)
...
if __name__=="__main__":
    ...
    parser.add_argument("--dataset", default="mnist", type=str, help="mnist | cifar10 | fashionmnist | cifar100 | shapes")
